...
-- 3. Highest total sales volume (by quantity)
...
-- 4. Supplier contribution to total revenue
SELECT s.sup_name,
       SUM(od.total_price) AS total_revenue
FROM supplier s
JOIN products p ON s.sup_id = p.sup_id
...
-- 3. Total sales processed per employee
SELECT e.emp_name,
...
-- 1. Quantity vs Total Price relationship
